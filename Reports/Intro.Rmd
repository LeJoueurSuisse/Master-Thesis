...
The aim of this work is to understand how the Swiss electricity grid works: we use it every day, but we do not know how it works, nor all the infrastructure put in place to keep it running smoothly. In Switzerland, the company Swissgrid is responsible for transmission throughout the country. As their data is public, we will be able to take an in-depth look at where, when and how electricity is produced and consumed in Switzerland. The flow analysis will be carried out in 4 stages:

- First, we will analyze the total data for the entire country.

- Then we will take a closer look at what is happening in each canton, and try to find similarities or differences between them.

- The final visualization research will focus on trade with neighboring countries, to determine who we depend on.

- We will finish with a quick modelling of the data, to try to forecast the data on two different timescales: a year ahead for monthly data, and a day ahead for daily data.

The data has revealed a strong seasonality of flows at three different levels: monthly, weekly and daily. Seasonality  is mainly governed by three factors: population, weather conditions and topography. However, we have found that these 3 variables did not have the same influence on consumption and production. In fact, topography has no direct impact on consumption, whereas it is essential for electricity production in Switzerland, which mainly comes from hydroelectric dams. The weather will have a strong impact on both consumption and production, but in the opposite way: when it is sunny and hot, the population will consume less electricity, whereas it is at this same period, with plenty of sunshine and water levels at their highest, that production is at its maximum. Finally, population will have a direct effect on consumption: even if we try to reduce per capita consumption, if the population increases, it will inevitably have a greater need for electricity. Its influence on production is indirect: since consumption and generation must always be in balance to guarantee grid stability, one will always have to adjust to the other.

We were able to demonstrate the influence of these three variables at cantonal level: only 5 cantons out of 26 produce the vast majority of electricity, which systematically comes from mountainous regions (with the exception of nuclear power plants, which account for 29% of Swiss production).

For consumption, we compared 2 maps of the Swiss cantons, filled in once for their population and a second time for their consumption, and noticed that the maps were almost identical. 

The data also showed us that Switzerland was not self-sufficient in its production and was obliged to buy from its neighbors in order to meet its needs during periods of high consumption. Of the four countries studied, we found that most of this electricity came from Germany, and very little from Italy.

Finally, the data available showed strong predictive power, both over 1 year and the previous day. In terms of models, Arima proved to be the best performer every time, giving us excellent results. It should be noted, however, that these models are purely statistical and do not take into account macroeconomic data such as new regulations or laws, climate change or possible shortages. 
...
In addition, Swissgrid needs to make its forecasts well in advance, so as to be well prepared for any eventuality. The first forecasts are made at one year, then at one month and one week before being recalculated one last time at 2 and 1 day before operation. You will find a section where we will try to make predictions about consumption and production using several models, and then assess their accuracy.
...
| Cantons' Data    | 5'049'198        | Combined Data for Cantons from 2015                 | Canton_df_long  |
| Borders data     | 2'244'088        | Data for import/export with borders country         | border_long     |
...
Same graph, but this time for the production :
...
With a range starting close to 0 (Appenzell), up to 1'500 (Argovie), the volatility among the cantons is much higher than for consumption. We can see from the second that 5 regions (Argovie, Berne/Jura, Grisons, Soleure and Valais) produce almost all the electricity in Switzerland.

The first graph shows trends and seasonal patterns. With the exception of Solothurn and Aargau, these are all identical, with a generally low level and production peaks in summer (as shown by the STL decomposition for the Valais). In terms of trends, the most noteworthy are those in Grisons, Berne/Jura and Tessin, which are down, and Glarus, which is up sharply after 2021 (as expected from the consumption analysis).

We are therefore going to take a closer look at the major producers, dividing them into two groups according to their behavior: 

Argovie and Soleure, with no trend and big drops during the summer.

Berne/Jura, Grison, Tessin and Valais, with a general downward trend and production peaks in summer. 

- First group : their energy is mainly produced in nuclear power plants. These are not affected by environmental conditions, so their seasonality is due to the human factor. Their production levels will simply balance out with consumption, hence the absence of a trend. In fact, nuclear power plants represent the second-largest source of electricity generation in Switzerland, second only to hydroelectric power. Since nuclear power can be used equally well all year round, the plants will boost their activity during peak consumption periods, i.e. in winter, when hydroelectric power stations produce the least.  Conversely, in summer, when less electricity is required, the plants will reduce their activity or even shut down for a short period (2 to 4 weeks) to carry out maintenance work and change the fuel. 

- Second group : This second group, mainly made up of Alpine cantons, draws its resources from hydroelectric activity. This activity depends directly on the water level in their dams or rivers, which is higher in summer when the snow melts and is marked by heavy rainfall. This activity depends directly on the water levels in their dams or rivers, which are higher in summer with the melting of snow and marked by heavy rainfall, thus explaining the summer peaks and winter troughs. Recent years have been marked by a drop in rainfall and a high level of drought, making it easy to explain a downward trend in production in these cantons.
...
As stated in the introduction, we are going to complete this work by making forecasts over two different time intervals: 1 year and 1 day. The idea is to use all the information we have shown in this paper to see if we can satisfactorily predict values for early 2023. For the one-year forecasts, we will use monthly data that we will draw up at the end of 2021, whereas we will use daily data for the one-day forecasts, which we will stop at December 31, 2022.

For each of our forecasts, we will use 3 models: TSLM, ARIMA and ETS. We will then compare their accuracy and show only the model with the best performance. 

To begin the forecast, we have carried out a brief analysis of the residuals. These revealed a seasonal pattern but without significance of the lag in ACF. The resisduals themselvess were white noise with a normal distribution, so we concluded that there was no need to transform the data. (we tried taking the log for TSLM, which showed no increase in perforormance)

## Monthly

For each model, we let R find the optimal parameters before running several tests to compare them. The parameters found "automatically" proved to be more efficient each time. Below is the performance table for the three models in question:
...
Minimizing the Root Mean Squared Error, we can see that the ARIMA model gives us the best results. 

Here is what the data looks like for an 18-months horizon :
...
arima.consm.fit %>%
  ...
  labs(title = "Monthly forecast using the Arima method",
       subtitle = "Ammount in millions of kWh",
       y = "", x = "")
```

As a reminder, the aim was to estimate consumption one year in advance. Although we used an 18-months period for the forecasts, it is the last 6 months (January to June 2023) that we are interested in. 

As the consumption data is very regular and follows the same pattern over the years, we can see that the model has given us very good forecasts with a fairly low confidence interval. 

We have followed exactly the same procedure for production forecasts and here is what our three models reveal: 
...
Again, using the RMSE as a scoring criterion, we will use the ARIMA model. 
...
Here too, forecasts seem perfectly in line with data. Despite less regular production in previous years, our models seem to have understood the right trends and seasonality. 

While the latter and the general level of data are satisfactory, we note a wider confidence interval than for consumption, due to greater data volatility since 2018.

## Daily

Our analysis of monthly data showed a preference for the arima model. These results were similar for daily data, where once again the arima model outperformed its competitors. 

The corresponding graphs for production and consumption are shown below:
...
The results are more than satisfactory for our two forecasts. Weekly seasonality has been well taken into account, and we note the same volatility as before. Regarding confidence intervals, the level seems lower than for monthly data, showing better precision when the time scale is reduced. 

\newpage

## Results and Limits

Regardless of the time scale used, the arima model performed better every time, with a very strong predictive power, both over 1 year and the previous day. 

For the other models, the difference is very small, especially in terms of fuel consumption. We did, however, notice a big increase in the confidence interval of the ETS, which we do not recommend. 
However, there are several limitations to our predictions: 

However, there are several limitations to our predictions: 
- economic and macroeconomic data have not been taken into account. Indeed, electricity may be subject to limits in order to preserve the planet, and its production could be strongly impacted by climate change. 

It might also have been interesting to compare our results with those directly obtained by Swissgrid. As the latter were not available to the public, we were unable to do so in this analysis. 

# Conclusion and possible extension

The aim of this project was to gain a better understanding of the Swiss electricity transmission network, managed by Swissgrid. More specifically, we wanted to see what regulated its consumption and production, the variables that influenced them, and the main agents in the sector (at cantonal level). The data was broken down to gobal level, cantonal, control energy and finally import/export. We were able to show the differences between consumption and production, as well as the different types of Swiss production and their specific features. 

We have also seen that network management and development are complex tasks that require a great deal of control and procedure. As electricity transport is a natural monopoly, Swissgrid is responsible for the entire network and everything that goes with it. In view of the ever-growing population and its ever-more demanding needs, and in a region that's becoming increasingly inaccessible, it's essential that we do everything we can to facilitate their expansion. As a reminder, an expansion project takes an average of 15 years to become operational. 
 
Regarding the data themselves, the key points to emerge were :

- the triple seasonality of the data, between day and night, weekday and weekend, summer and winter.

- inverse seasonality between consumption and production (summer/winter)

- Switzerland's two main production sources are hydroelectric and nuclear. As nuclear power is independent of the seasons, it is used to make up for shortfalls in production when consumption rises and hydroelectric dams are no longer sufficient.

- Even though Switzerland has a large number of production centers in the country, these are not sufficient, and the country is therefore dependent on its neighbors for its electricity consumption.

Our analysis has enabled us to draw up satisfactory forecasts for horizons of one year and one day. We have expressed various reservations and limitations concerning these forecasts, but without major changes in the environment or regulation, our forecasts are likely to be in line with reality.

We were able to show that the main variables influencing the network were population, topography and seasons. However, these causalities have only been shown and not proven with models. It would have been interesting to couple Swissgrid data with real macroeconomic data in order to model these relationships.

Another component, not mentioned in this paper, which has a major influence on elelectricity and its consumption is its price. With a war in Europe and several natural resources running out, we can expect the price to rise. Higher import and production prices could have a direct impact on consumption. An analysis of the evolution of electricity prices in Europe, or even worldwide, would merit further research. 
...
- https://www.bfe.admin.ch/bfe/fr/home/approvisionnement/energies-renouvelables/force-hydraulique.html#
- https://www.kkg.ch/fr/savoir/lectricite-issue-nergie-nucleaire/mix-electrique-suisse-69.html
