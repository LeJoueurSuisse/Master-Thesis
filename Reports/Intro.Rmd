...
Now that we have a better understanding of our data, we can go further and try to find the exact origin of these pattern. We will first analyze the STL decomposition on total sales, and then differentiate it for each department. As said before, we want to find out which specific event can be applied to each department : they are probably not all significant and may differ from on department to another.
You can see here the component of the STL on total sales (you can find the other in the appendix)
We find from the yearly seasonality : 


We can now build the stl decomp with additive paramater due to no change over time in the seasonlity :

We reduce the scope to four years the have a better a view of the data, we have shown that seasonality was constant over the year.
...
# Monthly production
general_dfM %>%
  group_by(month) %>% summarize(Total_prod = sum(energy_prod)) %>%
  as_tsibble() %>%
  filter_index(. ~ "2022-11-01") %>% 
  ACF(Total_prod) %>% 
  autoplot() +
